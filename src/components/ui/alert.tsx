...
const alertVariants = cva("flex gap-3 rounded-card p-4 text-sm leading-6 [&>svg]:mt-0.5 [&>svg]:size-5 [&>svg]:shrink-0", {
  variants: {
    variant: {
...
export function Alert({ className, variant, title, children, role, ...props }: AlertProps) {
  ...
  return (
    <div role={role ?? (urgent ? "alert" : "status")} className={cn(alertVariants({ variant }), className)} {...props}>
      <Icon aria-hidden />
